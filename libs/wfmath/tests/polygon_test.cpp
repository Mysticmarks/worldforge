...
void test_concave_polygon()
{
  ...
  REQUIRE(Intersect(p, Point<2>(0.5, 0.5), false));
  REQUIRE(!Intersect(p, Point<2>(1.5, 1.5), false));
}
...
void test_roundoff()
{
  ...
  REQUIRE(Intersect(near_vertical, box_v, false));
  REQUIRE(Intersect(near_vertical, Point<2>(5e-10, 0.5), false));
  AxisBox<2> box_v_out(Point<2>(-1e-9, 0.5), Point<2>(-5e-10, 0.6));
  REQUIRE(!Intersect(near_vertical, box_v_out, false));
  REQUIRE(!Intersect(near_vertical, Point<2>(-5e-10, 0.5), false));
  ...
  REQUIRE(Intersect(near_horizontal, box_h, false));
  REQUIRE(Intersect(near_horizontal, Point<2>(0.5, 5e-10), false));
  AxisBox<2> box_h_out(Point<2>(0.5, -1e-9), Point<2>(0.6, -5e-10));
  REQUIRE(!Intersect(near_horizontal, box_h_out, false));
  REQUIRE(!Intersect(near_horizontal, Point<2>(0.5, -5e-10), false));
}
...
void test_transformations()
{
  ...
  REQUIRE(Intersect(shifted, Point<2>(3.5, 4.5), false));
  REQUIRE(!Intersect(shifted, Point<2>(0.5, 0.5), false));

  Polygon<2> rotated = p;
  RotMatrix<2> rot;
  rot.rotation(0, 1, numeric_constants<CoordType>::pi() / 2);
  rotated.rotatePoint(rot, Point<2>(0, 0));
  REQUIRE(Intersect(rotated, Point<2>(-0.5, 0.5), false));
  REQUIRE(!Intersect(rotated, Point<2>(0.5, 0.5), false));
}
...
void test_polygon_with_hole()
{
  ...
  REQUIRE(Contains(outer, hole, false));

  Point<2> outsideHole(0.5f, -0.5f);
  REQUIRE(Intersect(outer, outsideHole, false));
  REQUIRE(!Intersect(hole, outsideHole, false));

  Point<2> insideHole(2.f, -2.f);
  REQUIRE(Intersect(outer, insideHole, false));
  REQUIRE(Intersect(hole, insideHole, false));

  bool compositeContains = Intersect(outer, insideHole, false) && !Intersect(hole, insideHole, false);
  REQUIRE(!compositeContains);
}

TEST_CASE("polygon_test")
{
  bool succ;
